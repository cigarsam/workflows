...
outputs:
  ...
  dedup_output:
    type: File
    label: "deduped PE bam file"
    outputSource: dedup_umi/output

  dedup_error_log:
    type: File
    label: "deduped BAM error log file"
    doc: "deduped BAM error log file"
    outputSource: dedup_umi/error_log

  dedup_log:
    type: File
    label: "deduped BAM log file"
    doc: "deduped BAM log file"
...
steps:
  ...
  extract_umi:
    ...
    in:
      ...
      bc_pattern:
        default: "NNNNNNNN"
    out: [output, output_pair, log, error_log]
...
#####dedup_umi:
  ...
  trim_fastq:
    ...
    in:
      ...
      input_file_pair: extract_umi/output_pair
      dont_gzip:
        default: true
      length:
        default: 30
  ...
  ##dont actually remove dup yet - deleted step from samtools
  samtools_sort_index_after_rmdup:
    run: ../tools/samtools-sort-index.cwl
    in:
      trigger: remove_duplicates
      sort_input: samtools_sort_index/bam_bai_pair
      threads: threads
    out: [bam_bai_pair]
  ...
  macs2_callpeak:
    ...
    in:
      treatment_file: dedup_umi/output
